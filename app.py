...
def exportar_excel_detalhado():
    ...
    with pd.ExcelWriter("relatorio_financeiro_detalhado.xlsx", engine="xlsxwriter") as writer:
        ...
        if not df_despesas.empty:
            ...
            categorias = df_despesas.groupby("categoria")["valor"].sum().reset_index()
            chart_despesas = workbook.add_chart({"type": "pie"})
            chart_despesas.add_series({
                "categories": ["Despesas", 1, 2, len(categorias), 2],
                "values": ["Despesas", 1, 1, len(categorias), 1],
                "name": "Despesas por Categoria"
            })
            chart_despesas.set_title({"name": "Despesas por Categoria"})
            worksheet_despesas.insert_chart("E2", chart_despesas)
